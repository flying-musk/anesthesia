...
async def regenerate_medical_translations():
    ...
    try:
        ...
        for mh in english_medical_histories:
            ...
            # 刪除現有的非英文版本（基於支持的語言列表）
            languages_to_delete = [lang.value for lang in medical_multilingual_service.supported_languages if lang != LanguageEnum.EN]
            db.query(MedicalHistory).filter(
                MedicalHistory.patient_id == mh.patient_id,
                MedicalHistory.group_id == mh.group_id,
                MedicalHistory.language.in_(languages_to_delete)
            ).delete()
        ...
        for sr in english_surgery_records:
            ...
            db.query(SurgeryRecord).filter(
                ...
                SurgeryRecord.language.in_(languages_to_delete)
            ).delete()
