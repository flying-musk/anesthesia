...
export default function GenerateGuidelinePage() {
  ...
  const STEPS = [
    getTranslation(currentLang, 'generateGuideline.steps.selectPatient'),
    getTranslation(currentLang, 'generateGuideline.steps.surgeryDetails'),
    getTranslation(currentLang, 'generateGuideline.steps.review')
  ];
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">
            {getTranslation(currentLang, 'generateGuideline.title')}
          </h1>
          <p className="text-muted-foreground">
            {getTranslation(currentLang, 'generateGuideline.description')}
          </p>
        </div>
      </div>
          ...
          <CardContent className="min-h-[400px]">
            ...
            {currentStep === 0 && (
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="patient_id">{getTranslation(currentLang, 'generateGuideline.form.selectPatient')} *</Label>
                  <Select
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue placeholder={getTranslation(currentLang, 'generateGuideline.form.selectPatient')} />
                    </SelectTrigger>
                    <SelectContent>
                ...
                {selectedPatient && (
                  <div className="rounded-lg border p-4">
                    <h4 className="mb-3 font-semibold">{getTranslation(currentLang, 'createPatient.form.fullName')}</h4>
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div>
                        <span className="text-muted-foreground">{getTranslation(currentLang, 'createPatient.form.fullName')}:</span>{' '}
                        {selectedPatient.full_name}
                      </div>
                      <div>
                        <span className="text-muted-foreground">{getTranslation(currentLang, 'createPatient.form.gender')}:</span>{' '}
                        {selectedPatient.gender === 'M' ? (currentLang === 'zh' ? '男' : 'Male') : 
                         selectedPatient.gender === 'F' ? (currentLang === 'zh' ? '女' : 'Female') : 
                         (currentLang === 'zh' ? '其他' : 'Other')}
                      </div>
                      <div>
                        <span className="text-muted-foreground">{getTranslation(currentLang, 'createPatient.form.phoneNumber')}:</span>{' '}
                        {selectedPatient.phone_number}
                      </div>
                      <div>
                        <span className="text-muted-foreground">{getTranslation(currentLang, 'createPatient.form.insuranceNumber')}:</span>{' '}
            ...
            {currentStep === 1 && (
              <div className="space-y-4">
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor="surgery_name">{getTranslation(currentLang, 'generateGuideline.form.surgeryName')} *</Label>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="surgery_type">{getTranslation(currentLang, 'generateGuideline.form.anesthesiaType')} *</Label>
                    <Select
                    ...
                    >
                      <SelectTrigger>
                        <SelectValue placeholder={getTranslation(currentLang, 'generateGuideline.form.anesthesiaType')} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="general">{getTranslation(currentLang, 'generateGuideline.anesthesiaTypes.general')}</SelectItem>
                        <SelectItem value="local">{getTranslation(currentLang, 'generateGuideline.anesthesiaTypes.local')}</SelectItem>
                        <SelectItem value="regional">{getTranslation(currentLang, 'generateGuideline.anesthesiaTypes.regional')}</SelectItem>
                        <SelectItem value="sedation">{getTranslation(currentLang, 'generateGuideline.anesthesiaTypes.sedation')}</SelectItem>
                      </SelectContent>
                    </Select>
                    {errors.surgery_type && (
                      <p className="text-sm text-red-500">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="surgery_date">{getTranslation(currentLang, 'generateGuideline.form.surgeryDate')} *</Label>
                    <Input
                      id="surgery_date"
                      type="date"
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="font-semibold text-blue-900">
                        {getTranslation(currentLang, 'generateGuideline.generating')}
                      </h4>
            ...
            <div className="mt-8 flex justify-between">
              ...
              >
                ...
                {getTranslation(currentLang, 'generateGuideline.buttons.previous')}
              </Button>

              {currentStep < STEPS.length - 1 ? (
                <Button type="button" onClick={handleNext}>
                  {getTranslation(currentLang, 'generateGuideline.buttons.next')}
                ...
                >
                  {generateGuideline.isPending ? (
                    ...
                    <>
                      ...
                      {getTranslation(currentLang, 'generateGuideline.buttons.generate')}
                    </>
                  )}
                </Button>
              )}
